...
public sealed class C2sReverseConversionTests
{
    ...
    [Fact]
    public async Task Writer_AirHoldContinuation_UsesAhdParentToken()
    {
        ...
        airHold.AppendChild(new PenguinTools.Chart.Models.umgr.AirHoldJoint
        {
            Tick = 480,
            Joint = Joint.C
        });
        ...
        var directory = Path.Combine(Path.GetTempPath(), "PenguinToolsTests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        var path = Path.Combine(directory, "ahd-parent.c2s");
        try
        {
            var written = await new C2SChartWriter(new C2SWriteRequest(path, converted.Value!))
                .WriteAsync(TestContext.Current.CancellationToken);
            Assert.True(written.Succeeded, written.ToString());

            var lines = await File.ReadAllLinesAsync(path, TestContext.Current.CancellationToken);
            var airHoldLines = lines
                .Where(line => line.StartsWith("AHD\t", StringComparison.Ordinal) ||
                               line.StartsWith("AHX\t", StringComparison.Ordinal))
                .ToArray();

            Assert.Equal(2, airHoldLines.Length);
            Assert.Contains("\tTAP\t", airHoldLines[0]);
            Assert.Contains("\tAHD\t", airHoldLines[1]);
            Assert.DoesNotContain("\tAHX\t", airHoldLines[1]);
        }
        finally
        {
            Directory.Delete(directory, true);
    ...
    [Fact]
    public async Task Writer_AirHoldOnExHold_UsesHxdParentToken()
    {
        var chart = new PenguinTools.Chart.Models.umgr.Chart();
        chart.Meta.MainBpm = 120m;
        ...
        hold.AppendChild(new PenguinTools.Chart.Models.umgr.HoldJoint { Tick = 480 });
        ...
        var tail = Assert.IsType<PenguinTools.Chart.Models.umgr.HoldJoint>(hold.LastChild);
        tail.MakePair(airHold);
        ...
        var directory = Path.Combine(Path.GetTempPath(), "PenguinToolsTests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        var path = Path.Combine(directory, "ahd-hxd.c2s");
        try
        {
            var written = await new C2SChartWriter(new C2SWriteRequest(path, converted.Value!))
                .WriteAsync(TestContext.Current.CancellationToken);
            ...
            var ahd = Assert.Single(
                await File.ReadAllLinesAsync(path, TestContext.Current.CancellationToken),
                line => line.StartsWith("AHD\t", StringComparison.Ordinal));
            Assert.Contains("\tHXD\t", ahd);
            Assert.DoesNotContain("\tHLD\t", ahd);
        }
        finally
        {
            Directory.Delete(directory, true);
    ...
    [Fact]
    public async Task Writer_OfficialTapAirHold_MatchesGameTokenOrder()
    {
        var directory = Path.Combine(Path.GetTempPath(), "PenguinToolsTests", Guid.NewGuid().ToString("N"));
        var path = Path.Combine(directory, "official-ahd.c2s");
        Directory.CreateDirectory(directory);
        await File.WriteAllTextAsync(path, """
            VERSION	1.12.00	1.12.00
            RESOLUTION	384
            TAP	0	0	4	4
            AHD	0	0	4	4	TAP	96	DEF
            """, TestContext.Current.CancellationToken);
        ...
        var roundTrip = new C2SChartConverter(new C2SConvertRequest(ugc.Value!)).Convert();
        Assert.True(roundTrip.Succeeded, roundTrip.ToString());

        var output = Path.Combine(directory, "roundtrip.c2s");
        var written = await new C2SChartWriter(new C2SWriteRequest(output, roundTrip.Value!))
            .WriteAsync(TestContext.Current.CancellationToken);
        Assert.True(written.Succeeded, written.ToString());

        var ahd = Assert.Single(
            await File.ReadAllLinesAsync(output, TestContext.Current.CancellationToken),
            line => line.StartsWith("AHD\t", StringComparison.Ordinal));
        Assert.Equal("AHD\t0\t0\t4\t4\tTAP\t96\tDEF", ahd);
    }
